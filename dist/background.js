...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // 在 background.js 中修改 fetchNearbyCoffee 部分
  if (request.action === "fetchNearbyCoffee") {
    ...
    const query = `
      ...
      (
        ...
        nwr["name"~"coffee|café|cafe|咖啡",i](around:${radius},${currentCoords[0]},${currentCoords[1]});
      );
      out center;
        `;
    ...
    fetch(apiUrl, {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          return response.text().then((text) => {
            console.error(
              "[Background] Overpass API error response text:",
              text
            );
        ...
        if (!contentType || !contentType.includes("application/json")) {
          return response.text().then((text) => {
            console.error(
              "[Background] Overpass unexpected content type:",
              contentType,
              "Response text:",
              text
            );
            throw new Error(
              `Overpass API returned non-JSON response. Content-Type: ${contentType}`
            );
          });
        }
      ...
      .then((data) => {
        if (!data || !data.elements) {
          console.warn(
            "[Background] Invalid data structure received from Overpass:",
            data
          );
        ...
        if (data.elements.length === 0) {
          console.warn(
            "[Background] No coffee shops found via Overpass for coords:",
            currentCoords
          );
          ...
          const formattedData = data.elements
            .map((element) => {
              ...
              if (element.type === "node") {
                ...
                console.warn(
                  "[Background] Element skipped, missing coordinates:",
                  element.type,
                  element.id
                );
                return null;
          ...
          console.log(
            "[Background] Sending formatted data count:",
      ...
      .catch((error) => {
        ...
        sendResponse({
          ...
          error:
            error.message ||
            "Failed to fetch from Overpass API. Check background logs.",
        });
      });
